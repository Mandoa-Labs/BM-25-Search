...
* `--additional-packs` tells CodeQL where to find the pack on disk. For more information, see [database run-queries](/en/code-security/reference/code-scanning/codeql/codeql-cli-manual/database-run-queries).
...
For more information, see [Workflow configuration options for code scanning](/en/code-security/code-scanning/creating-an-advanced-setup-for-code-scanning/customizing-your-advanced-setup-for-code-scanning#filtering-the-queries-in-a-codeql-pack).
...
2. **On pull requests:** Download the cached overlay-base database, then create a lightweight "overlay" database that only processes the changed files.
