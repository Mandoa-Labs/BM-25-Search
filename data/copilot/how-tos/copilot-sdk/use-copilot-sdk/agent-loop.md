...
Learn how Copilot CLI processes a user message end-to-end, from prompt to session.idle.
...
1. Copilot CLI sends the conversation history to the LLM
2. The LLM responds (possibly with tool requests)
3. If tools were requested, Copilot CLI executes them
...
1. The model calls `task_complete` with a summary → Copilot CLI emits `session.task_complete` → done
2. The model stops without calling it → Copilot CLI nudges → model continues or calls `task_complete`
