...
You can use the ghes-manage API or es plugin to automate upgrade operations in Enterprise Server environments.

You can upgrade a GitHub Enterprise Server instance using the Manage GitHub Enterprise Server API or the `gh es` CLI extension. These tools automate the process of downloading the upgrade package, running pre-upgrade checks, and applying the new version.
...
* Ensure you have authentication credentials for the Manage GitHub Enterprise Server API. For more information, see [REST API endpoints for GitHub Enterprise administration](/en/enterprise-server@3.21/rest/enterprise-admin#authentication).
